...
\begin{abstract}
海上搜救任務面臨著範圍廣闊、海象險惡、時間緊迫等嚴峻挑戰。為應對此一難題，本專案建構了一套「智慧化船舶群集搜救模擬系統」。系統核心旨在模擬並最佳化真實的搜救作業流程：首先，依據目標可能的出現區域進行分割；接著，指揮調度多艘船隻構成的搜救群集，對各區塊展開平行搜索，以最大化覆蓋率並縮短搜尋時間。我們在 Unity 與 Crest 打造的擬真海洋環境中，採用 SAC 深度強化學習演算法，賦予每艘搜救船隻在複雜風浪中自主執行精密搜尋、並規避動態障礙物的能力。本專案旨在評估此 AI 驅動的搜救群集，在不同搜救情境下，對於提升目標發現成功率、縮短搜救時間的實際成效，並為未來智慧化海上應急響應系統提供關鍵的模擬驗證。
\end{abstract}
...
\item \textbf{第二章　文獻回顧與討論}：探討海上搜救、強化學習、多代理人系統與模擬相關的過往研究。
\item \textbf{第三章　相關使用技術介紹}：介紹本研究所採用之深度強化學習演算法（SAC）、Unity3D、ML-Agents 與 Crest 模擬套件。
\item \textbf{第四章　系統設計與技術實作}：說明系統硬體與軟體環境、系統架構設計與強化學習架構。
...
隨著無人化技術的推進，自主船舶在海洋產業中逐漸成熟並投入實際應用。主要集中在以下的面向：
\begin{itemize}
...
\begin{enumerate}
	...
	\item \textbf{Critic（價值網路）}：使用兩個 Q 函數（double Q-networks）以降低過度估計偏差，透過軟化的貝爾曼方程（soft Bellman backup）學習回報與熵加權後的價值。
...
\begin{itemize}
    ...
    \item \textbf{C\# 腳本語言}：使用 C\# 編寫遊戲邏輯行為，並支持與外部系統進行資料交換。
...
ML-Agents（Unity Machine Learning Agents Toolkit）是 Unity 官方推出的開源工具套件，主要用於將 Unity 環境轉化為可供智能體（Agent）訓練的模擬環境，並進行機器學習或強化學習的訓練。
\\ \par
ML-Agents 的核心特點包括：
...
\subsubsection{核心特點}
\begin{itemize}
    \item \textbf{逼真波浪模擬}：生成自然海浪，可模擬遠景大波浪和近景小波紋。
...
以下三張圖片是搜尋任務的執行情況，圖片中的方框為落海者的可能範圍，紅點為實際遇難者的位置，而不同的顏色則代表在搜尋過程中各個位置被不同船隻的搜尋範圍所搜尋的次數，船隊由圖片左下角出發，進入紅框開始搜尋。

\begin{figure}[h]
    \centering
    \begin{minipage}[t]{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/SingleInRangeSuccess.png}
        \caption{範圍內搜尋成功案例}
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/SingleOutOfRangeSuccess.png}
        \caption{範圍外搜尋部分成功}
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/SingleOutOfRangeFail.png}
        \caption{範圍外搜尋失敗案例}
    \end{minipage}
\end{figure}

我們一共進行了 359 次測試，其中 295  次成功，64  次失敗，整體救援率高達 82\%。由於所有船隻的偵測範圍幾乎掃過整個方框範圍，顯示搜尋的覆蓋率相當完整，幾乎涵蓋了所有的可能落水區域，甚至能找到漂流至方框外的落水者。此結果顯示，我們設計的系統確實能夠在廣闊且情況未知的海域中，以相對低廉的成本的執行平行化搜尋，並有效定位落水者。雖然存在一定比例的失敗案例，但比例有限，且整體數據充分證明系統在本情境中的可行性。
