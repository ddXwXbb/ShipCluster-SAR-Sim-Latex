...
\usepackage{multirow}
\usepackage{placeins}
% Improve float placement tolerance so LaTeX can place more floats on the page
...
\hline
黃品翰 & 海洋環境架設、進行訓練、訓練環境架設、模型配置設計與調校、具象化實驗結果、撰寫相關文件、整理實驗結果 & 25\% \\
\hline
唐予安 & 撰寫相關文件、進行訓練、獎勵函數設計、訓練流程規劃、整理相關文獻 & 25\% \\
\hline
馮宥崴 & 調查海上搜救方法、整理相關文獻、海洋環境架設、搜救實驗規劃、撰寫相關文件、整理實驗結果 & 25\% \\
\hline
巫侑霖 & 系統架構規劃、核心模組程式碼實作、查閱技術文件、模型配置設計與調校、進行模型訓練、進行實驗& 25\% \\
\hline
\end{tabular}
\end{table}
...
\item \textbf{設計群集搜尋系統}：設計一套系統，得以根據已知的資訊進行搜救規劃，並統籌無人船進行搜尋行動。
...
\subsection{實驗方法與結果}
本研究在 Unity 與 Crest 海洋物理引擎所建構的模擬環境中，使用 SAC 深度強化學習演算法訓練自主船舶，以具備導航與避障能力。雖然整體系統設計為多代理架構，但為降低訓練複雜度與收斂時間，本研究採用單代理訓練，並利用輔助船隻模擬多代理互動，以確保船舶能在動態環境中學習穩定航行與避障策略。
...
\paragraph{案例分析}
\begin{itemize}
  \item DQN 與 PPO 演算法避障模擬之比較分析：一項研究探討 DQN 和 PPO 這兩種強化學習 (RL) 演算法，在 2D 模擬環境中引導無人機 (UAV) 避障的性能，最終發現在 2D 環境中，PPO 在所有指標上都明確超越 DQN。\cite{rybchak2024}。
...
\begin{figure}[!htbp]
    ...
    \begin{minipage}[t]{0.3\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/DQN\_PPO\_NavDemo.png}
        \caption{DQN 與 PPO 在導航策略上差異示意圖\cite{rybchak2024}}
    \end{minipage}%
    \hspace{1cm}
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/AutoDroneDemo.png}
        \caption{無人機應急導航，利用 OpenStreetMap 訓練，再利用 UE4 視覺化\cite{greenberg}}
    \end{minipage}%
\end{figure}
...
\begin{itemize}
  \item \textbf{研究缺口}：缺乏針對「智慧化船舶群集」在海洋環境下進行搜救的規劃系統平台。
  \item \textbf{本研究貢獻}：
  \begin{itemize}
    \item 建立結合 Unity 與 Crest 的海洋高擬真測試平台。
    \item 開發基於 SAC 的自主船舶代理人。
    \item 製作出可行的搜救規劃系統，並透過模擬驗證效能。
  \end{itemize}
\end{itemize}
...
本研究的系統採用前後端分離的設計架構。此架構分為兩大核心：

\begin{itemize}
    \item \textbf{前端操作介面 (Frontend)}：基於 React 框架開發，負責地圖視覺化、用戶操作指令的發送、以及即時模擬狀態的顯示。
    \item \textbf{後端搜救規劃 (Backend)}：利用 C\#、Python 開發，負責獲取資料、規劃搜救及統籌海流資料。
\end{itemize}

兩者透過 API 服務層進行非同步通訊，前端以 100ms 的頻率輪詢後端狀態，並在用戶操作時發送指令，實現即時互動。

\subsubsection{前端介面}

前端系統負責所有的用戶互動與視覺化任務。如圖 \ref{fig:frontend_arch} 所示，其核心組件包含主應用程式、地圖模組、與控制面板，並透過 API 層與後端溝通。

\begin{figure}[!htbp]
    \centering
    \begin{minipage}[t]{1\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/FrontendArch.png}
        \caption{前端系統架構圖}
        \label{fig:frontend_arch}
    \end{minipage}
\end{figure}

\paragraph{地圖視覺化 (Map Visualization)}
\begin{itemize}
    \item \textbf{地圖互動}：支援縮放、拖曳、點擊等操作。
    \item \textbf{標記系統}：視覺化顯示船隻、遇險人員的即時位置，以及船隻的偵測範圍圈。
    \item \textbf{海流可視化}：以箭頭顯示海流方向與強度，並透過流動粒子動畫呈現海流動態。
    \item \textbf{船隻軌跡}：記錄並顯示船隻的移動歷史路徑。
\end{itemize}

\paragraph{數據管理層 (State Management)}
\begin{itemize}
    \item \textbf{狀態管理}：統一管理船隻數據、遇險人員、航點等全域狀態。
\end{itemize}

\paragraph{API 服務層 (API Service)}
提供與後端通信的介面：
\begin{itemize}
    \item \textbf{getStatus}：獲取系統即時狀態。
    \item \textbf{reset}：發送重置模擬的指令。
    \item \textbf{sendCommand}：發送船隻啟動/停止等指令。
    \item \textbf{sendWaypoints}：發送用戶錄製的航點數據。
    \item \textbf{spawnPersons}：在指定位置設定待救者數量及範圍。
\end{itemize}

\paragraph{資料流 (Data Stream)}
\begin{itemize}
    \item \textbf{輪詢更新}：負責每 100ms 向後端請求更新前端狀態。
\end{itemize}

\paragraph{側邊選單 (Side Menu)}
\begin{itemize}
    \item \textbf{船隻控制}：控制船隻的啟動與停止狀態。
    \item \textbf{航點錄製}：提供使用者自行選擇航點的功能，透過在地圖上面點選以設定航點。
    \item \textbf{視角追蹤}：可選擇追蹤特定船隻或切換至全域視角。
    \item \textbf{自動搜救}：按下後，系統會指派船隻出港，進行自主搜救。
\end{itemize}

\paragraph{船隻儀表板 (Boat Dashboard)}
\begin{itemize}
    \item \textbf{狀態}：個別船隻目前的等待狀態。
    \item \textbf{經緯度}：個別船隻目前的經緯度。
    \item \textbf{移動軌跡}：個別船隻的航行軌跡，可自主選擇要不要顯示。
\end{itemize}

\paragraph{監控視窗 (Picture-in-Picture)}
\begin{itemize}
    \item \textbf{獨立視窗}：提供獨立的迷你地圖視窗，可同時監控不同區域。
    \item \textbf{船隻追蹤}：可選擇追蹤特定船隻，視窗自動跟隨船隻移動。
    \item \textbf{縮放控制}：可與大地圖不同、獨立的縮放控制。
\end{itemize}

\newpage
\subsubsection{前端介面展示}

\begin{figure}[!htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/FrontendDemoAtDock.png}
        \caption{船隻於港內等待}
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/FrontendDemoDeparture.png}
        \caption{船隻出發搜尋}
    \end{minipage}%
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/FrontendDemoCurrent.png}
        \caption{海流可視化展示}
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/FrontendDemoPath.png}
        \caption{船隻軌跡展示}
    \end{minipage}%
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \begin{minipage}[t]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/FrontendDemoDashboard.png}
        \caption{船隻儀表板介面}
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/FrontendDemoPIP.png}
        \caption{監控視窗展示}
    \end{minipage}%
\end{figure}
...
\subsection{環境架構}
為了有效訓練及驗證本研究所提出的搜救系統，我們使用 Unity 建構了一個高擬真的模擬環境。本章節將詳細闡述此環境的架構設計。

如圖 \ref{fig:env} 所示，整體架構被劃分為七大核心系統，各自負責專門的功能，並透過明確的資料介面協同運作。此架構以中央控制系統為頂層，往下管理船舶管理系統、波浪系統、障礙物、風力及遇難者等關鍵子系統。

此外，資料輸出模組作為一個 API 平台，讓我們的系統能與模擬環境進行即時的資料交互。以下將詳細介紹各個模組的功能。

\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.6]{image/EnvArchitecture.png}
    \caption{環境架構示意圖}
    \label{fig:env}
\end{figure}
...
\begin{itemize}
	...
	\begin{itemize}
	    \item \textbf{輔助船隻 (Zombie Boat)}：配備隨機動作選擇器與動作模組，作為訓練模式中的虛擬代理。
	...
	\item \textbf{資料輸出 (Data Output)}
	\begin{itemize}
	   \item 為 HTTP API 平台，系統可透過其向環境發送請求，以改變船隻狀態或是取得資料。
	\end{itemize}	
...
\begin{itemize}
    \item 鄰近船隻狀態 (6m 維)：環境中假設有鄰近的 m 艘船，對每艘鄰近船隻記錄相對方位角正弦/餘弦、距離、航向角正弦/餘弦、徑向速度。
\end{itemize}
...
\begin{itemize}
    ...
    \]
船隻是否朝著目標前進，向目標靠近越多獎勵值越高，向目標遠離則反之。將獎勵值限制在 $[-R_D, R_D]$ 之間，避免單一步長中距離改變過大導致獎勵失控。
    \item \textbf{航向獎勵 (Heading Reward)}：
    \[
    r_{\text{head}} = R_H \cdot (f \cdot u)
    \]
船隻是否面對著目標點，完全面對目標點獎勵最高，背離目標則反之。當船隻正對目標時為 $R_H$，背離目標時為 $-R_H$。
    \item \textbf{障礙物懲罰 (Proximity Penalty)}：
    \[
    r_{\text{prox}} = R_P^{\max} \cdot \max\!\left(0, \frac{W - m_t}{W}\right)
    \]
當船隻與最近的障礙物距離 $m_t$ 小於警戒距離 $W$ 時，施加懲罰。
\end{itemize}
...
\begin{itemize}
    ...
    r_{\text{time}} = \!\left(\frac{T_{\max}-s}{T_{\max}}\right) \cdot R_{\text{tb}}^{\max}, \quad d_t<d_{\text{succ}}
    \]
成功抵達目標時獲得基礎成功獎勵 $R_{\text{succ}}$，並加上時間獎勵 $r_{\text{time}}$，鼓勵船隻在更短的步數內完成任務。
    \item \textbf{碰撞 (Collision Penalty)}：
    \[
    R_{\text{col}}, \quad m_t < m_{\text{col}}
    \]
船隻與障礙物的距離小於碰撞閾值 ($m_{\text{col}}$) 時，給予懲罰 $R_{\text{col}}$。
    \item \textbf{超時 (Timeout Penalty)}：
    \[
    R_{\text{to}}, \quad s > T_{\max}
    \]
任務步數超過最大上限 ($T_{\max}$) 時，給予懲罰 $R_{\text{to}}$。
\end{itemize}
...
R_{\text{col}}, & m_t < m_{\text{col}}\\
R_{\text{to}}, & s > T_{\max}\\
R_{\text{succ}} + \displaystyle \!\Big(\frac{T_{\max}-s}{T_{\max}}\Big)\cdot R_{\text{tb}}^{\max}, & d_t < d_{\text{succ}}\\
0, & \text{otherwise}
\end{cases}
\end{aligned}
\]
...
\subsubsection{搜尋判斷機制}
為模擬真實搜救情境中的不確定性及因距離對能見度造成的影響，本系統的搜尋機制並非採用簡單的二元範圍觸發，而是導入了機率式的偵測模型。

每艘自主船舶皆配有一個預定的搜救範圍。當遇難者進入此範圍內時，系統並不會立即將其標記為「已尋獲」。相反地，系統會在 \textbf{每一個模擬時間步}中，根據船隻與遇難者之間的 \textbf{即時距離}，重新計算一個「瞬時偵測成功率」。

此機率是透過一個反比衰減函數（inverse decay function）來計算，其核心邏輯如您所提供的函式，可表示為以下方程式：

$$ P_{\text{detect}} = \frac{1}{1 + (d / d_0)} $$

其中：
\begin{itemize}
    \item $P_{\text{detect}}$：代表單一時間步內，偵測成功的機率。
    \item $d$：船隻中心點與遇難者之間的當前距離（單位：公尺）。
    \item $d_0$：基礎偵測距離參數，在本系統中設定為 \textbf{5.0}。
\end{itemize}

此公式的意義在於：
\begin{itemize}
    \item 當船隻與遇難者距離為 0 時，$d=0$，瞬時機率 $P_{\text{detect}} = 1.0$ (100\%)。
    \item 當距離 $d$ 等於基礎參數 $d_0$（5 公尺）時，機率 $P_{\text{detect}} = 1 / (1 + 1) = 0.5$ (50\%)。
    \item 隨著距離 $d$ 增加，瞬時偵測機率會下降，模擬出感測器在遠距離時的效能限制。
\end{itemize}

在模擬過程中，只要遇難者位於船隻的搜尋範圍內，系統便會持續在每一刻進行此機率判斷。遇難者被正式標記為「已尋獲」的事件，發生在\textbf{第一次}此判斷回傳為「成功」的時刻。這種設計能更真實地反映搜救作業：位於範圍邊緣的目標可能需要船隻多次掃過才能成功定位，而近距離的目標則會被迅速鎖定。

\begin{figure}[!htbp]
    \centering
    \begin{minipage}[t]{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{image/SearchingProb.png}
        \caption{搜尋成功率函數圖形}
    \end{minipage}
\end{figure}
...
\subsection{實驗架構設計}
首先，在訓練階段中，系統將環境中的「遇難者系統」 關閉，並建立一個具備隨機障礙物與輔助船隻的模擬場景。我們會使用單一代理人訓練的方式，透過深度強化學習進行訓練，學習導航與避障策略，以確保其能在複雜的海況下保持穩定航行並避免碰撞。
\\ \par
接著，在模擬測試階段，我們於場景中部署多艘自主船舶，這些船舶均掛載前一階段所完成的訓練模型。在這個階段中，我們移除了輔助船隻，並引入遇難者系統以生成需要救援的目標，搭配風浪與障礙物，重現真實的救援搜尋場景。船舶群集在此階段能同時執行搜救任務，展現協作與分工行為。於此過程中，系統持續記錄各項效能指標數據，以供後續統計與分析之用。
\\ \par
...
本實驗的訓練環境以 Unity 與 Crest 海洋物理引擎為基礎，模擬出真實的海面，包括風浪、風速及動態的障礙物，以提供船隻導航及避障所需的複雜環境。我們在場景中配置一個訓練船隻，並加入多艘輔助船隻作為其他代理人的替代品，這些船隻會隨機行動，模擬多代理系統的互動行為。
值得注意的是，雖然整體系統設計為多代理人系統，但直接訓練多代理人系統會使環境過於複雜，使收斂時間過長，甚至可能無法收斂，而我們的訓練目標僅是船隻的避障與導航，不會涉及代理間的合作。因此我們採用了單代理人的訓練方式，結合上述的輔助船隻模擬多代理環境，既能節省資源，又能讓代理學習多代理系統下的避障與導航策略。
...
\begin{enumerate}
    \item \textbf{環境初始化}：配置船隻、目標點、障礙物與 輔助船隻。
    \item \textbf{深度強化學習訓練}：透過 ML-Agents 啟動訓練流程，並採用 SAC 演算法進行訓練，訓練目標為讓船隻從初始位置移動到目標點，同時，在途中不能碰到障礙物或是輔助船隻。
...
\begin{itemize}
	...
	\item \textbf{設計搜救規劃系統}：設計一個系統，針對多艘船舶進行協同搜尋與任務的分配，以提升搜救效率。
...
\subsubsection{專題成果}
\begin{itemize}
	\item \textbf{系統面成果}：本系統提供一個視覺化的平台，使搜救人員能直觀的使用無人船執行搜救任務，並且船隻可以根據風向及海流改變搜尋路徑，進而搜尋到目標。
	
	\item \textbf{實驗或應用面成果}：單一落水快速救援的環境中，在系統的加持下救援成功率高達 82\%，並且成功預測了 81\% 的人員位置，相較於傳統人工搜救 50\% 的成功率，具有明顯的優勢。
	
	\item \textbf{理論或方法面成果}：本研究在演算法與方法上，實現了基於環境感知的導航與避障策略，使船隊能在動態海域環境中自主調整路徑，避免碰撞並保持最佳搜尋覆蓋率。
	
	\item \textbf{應用與價值}：本系統具備在大型船難、空難迫降或軍事與民用搜救等情境下的應用潛能。其低成本、高效率的搜尋方式，能減少人力依賴並提升救援成功率，對於提升整體搜救資源運用效率具有重要價值。未來若能結合真實感測數據與更複雜的環境建模，將有助於推動自主搜救技術在實務上的落地與推廣。
\end{itemize}

\subsubsection{研究限制}
\begin{itemize}
	\item \textbf{實驗設計限制}：由於本研究主要在模擬環境中進行，無法涵蓋真實海域中可能出現的各種不確定性，例如：雷擊等特殊情況。實驗中所設定的參數僅能代表部分典型情境，因此結果雖具參考價值，但仍需進一步實地驗證。  

	\item \textbf{方法論限制}：在嘗試多代理訓練時，由於環境複雜或演算法設計限制，訓練過程往往難以收斂且穩定性較低。因此本研究採用單代理訓練方式，雖能在系統統籌下有效完成多代理搜尋任務，但因此缺少多代理協作所帶來的潛在優勢。這也是未來研究需要進一步探討的方向。


	\item \textbf{時間與資源限制}：受限於研究時間與資源，本研究無法取得實際的船舶設備或海上場域進行驗證，相關實驗僅止於模擬層面。

\end{itemize}

\subsection{未來研究方向}
\begin{itemize}
	\item \textbf{多代理協作訓練}：引入多代理強化學習或協作演算法，提升船隊在複雜環境下的協作效率與搜尋成功率。針對多代理訓練難以收斂的問題，可探索分層控制、經驗共享或穩定化策略，改善訓練穩定性。

	\item \textbf{更真實的環境}：優化模擬環境並加入更多變因，提升系統的真實性。

	\item \textbf{更多搜尋策略}：加入更多搜尋策略，讓系統可以適應不同規模的任務性質及搜救任務，增加系統的應用範圍。

	\item \textbf{連接到實體測試}：將系統應用在實際的水面上，結合實時風向、海流及各種感測數據進行搜救測試，驗證其可靠性。
\end{itemize}


\subsection{可行的解決方案}
\begin{itemize}
	\item \textbf{多代理協作改進}：採用分層控制或經驗共享策略進行多代理訓練，可減少訓練不收斂的風險，提升船隊在動態環境下的協作效率。透過模擬不同海域與任務規模，逐步調整協作演算法，達成穩定且可擴展的多代理系統。

	\item \textbf{環境模擬精度提升}：引入更精細的海流、風向以及潮汐資料，改善模擬精度，使系統在不同環境變化下能更準確地預測落水者位置。可使用歷史數據或即時資料進行校正，縮小模擬與現實的差距。
\end{itemize}
